...
class TrainRegularizationUtils(object):
            ...
            for phase in ['train', 'val']:
                ...
                for batch_idx, (inputs, labels) in enumerate(self.dataloaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if args.method == 'label_smooth':
                            loss = self.LS_criterion(outputs, labels)
                        elif args.method == 'Vanilla':
                            loss = self.criterion(outputs, labels)
                        elif args.method == 'focal_loss':
                            loss = self.focal_loss(outputs, labels)
                        elif args.method == 'confidence':
                            loss = self.criterion(outputs, labels) - 0.15 * self.confidence_loss(outputs)
