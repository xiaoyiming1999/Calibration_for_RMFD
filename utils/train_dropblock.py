...
class TrainDropBlockUtils(object):
    ...
    def setup(self):
        ...
        # define model
        self.model = resnet18_with_dropblock(out_channel=args.num_classes)
            ...
            for phase in ['train', 'val']:
                ...
                for batch_idx, (inputs, labels) in enumerate(self.dataloaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if phase == 'val':
                            all_MC_outputs = []
                            for t in range(args.num_MC_samplings):
                                MC_outputs = self.model(inputs)
                                MC_outputs = MC_outputs.unsqueeze(dim=1)
                                all_MC_outputs.append(MC_outputs)
                            outputs = torch.cat(all_MC_outputs, dim=1)
